...
void regionGrowing(const pcl::PointCloud<pcl::PointXYZRGBNormal>::Ptr& cloud,
    ...
    std::vector<int> seedPoints;
    if (seedIndices.empty()) {
        for (int i = 0; i < 10 && i < static_cast<int>(cloud->size()); ++i) {
            seedPoints.push_back(i);
        }
    } else {
        seedPoints = seedIndices;
...
int main(int argc, char** argv) {
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZRGBNormal>(inputFile, *cloud) == -1) {
    ...
    int k_1 = 50;
    float km = 0.2f;
    float normalThreshold = ((1.0 * static_cast<float>(PI_CONST)) / 180.0f);
    ...
    float cvs_0=0;
    cvs_0=cvs;
    for (int iter = 0; iter < max_iters && cvs >= 1.6f && cvs_0 >= cvs; ++iter ) {
        outputClouds.clear();
        outputClouds_w.clear();
        seedIndices.clear();
    ...
    // 保存输出
    if (!cloud_new->empty()) {
        pcl::io::savePCDFileBinary(outputFile, *cloud_new);
        std::cout << "Saved: " << outputFile << std::endl;
    } else {
        std::cout << "cloud_new is empty; nothing saved." << std::endl;
    }

    pcl::visualization::PCLVisualizer::Ptr viewer(new pcl::visualization::PCLVisualizer("Final Output"));
    viewer->setBackgroundColor(0, 0, 0);

    // 使用点云本身的 RGB 颜色
    pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGBNormal> rgb(cloud_new);
    viewer->addPointCloud<pcl::PointXYZRGBNormal>(cloud_new, rgb, "cloud_new");
    viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "cloud_new");
